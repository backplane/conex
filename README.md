...
[grta](grta) | single-binary container image which provides an HTTP endpoint which receives webhooks and writes the PSK-validated payloads to disk | [Dockerfile](grta/Dockerfile) | [galvanist/conex:grta](https://hub.docker.com/r/galvanist/conex/tags?name=grta)
...
[myip](myip) | single-binary container image which provides an HTTP endpoint which returns the user's own IP address in JSON format | [Dockerfile](myip/Dockerfile) | [galvanist/conex:myip](https://hub.docker.com/r/galvanist/conex/tags?name=myip)
